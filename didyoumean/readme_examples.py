...
def main():
    ...
    examples = {
        (1, NameError): {
            ...
            (3, "Looking for missing imports"): ["string.ascii_lowercase",],
            (4, "Looking in missing imports"): ["choice",],
            (5, "Special cases"): ["assert j ** 2 == -1",],
        },
        (2, AttributeError): {
            ...
            (3, "Detection of mis-used builtins"): ["lst = [1, 2, 3]\nlst.max()",],
            (4, "Period used instead of comma"): ["a, b = 1, 2\nmax(a. b)",],
        },
        (3, ImportError): {
            (1, "Fuzzy matches on existing modules"): ["from maths import pi",],
            (2, "Fuzzy matches on elements of the module"): ["from math import pie",],
            (3, "Looking for import from wrong module"): ["from itertools import pi",],
        },
        (4, TypeError): {
            ...
            for code in codes:
                ...
                if exc is None:
                    ...
                    if not issubclass(type_, exc_types):
                        before = after = (
                            "Wrong exception thrown on this version of Python (%s != %s)"
                            % (type_, exc_types)
                        )
                    else:
